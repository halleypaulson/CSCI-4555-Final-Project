...
model %>% compile(
  loss = 'mean_squared_error', #changing this from mean_absolute_error didn't change the loss measurement, but this is the ideal loss function
  optimizer = 'adam',
) #how do we add back propagation? or drop rates?
...
```
Loss decreased with another layer and more epochs, but it doesn't change the actual predictions much. How can we improve the accuracy and reduce the loss? Maybe adding a drop rate or implementing back propagation if that isn't already happening.
...
head(comparison)
summary(comparison) #looks like it's not so good at predicting lower or higher, but good at predicting around the median
